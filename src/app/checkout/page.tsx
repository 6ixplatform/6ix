...
const USD_MONTHLY = { pro: 6.66, max: 16.66 } as const; // base USD list price
const YEARLY_FACTOR = 10; // 2 months free (12 mo for 10x price)
...
function useLocalPrice(): LocalPrice {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                // You already have these endpoints in your project.
...
export default function CheckoutPage() {
    ...
    // local currency preview (client side)
    ...
    // server-locked quote (what the gateway will charge)
        ...
        try {
            const res = await fetch('/api/billing/initiate', {
                ...
                body: JSON.stringify({
                    ...
                    // tell the server our detected currency (server still validates / converts)
            ...
            // lock header to server-quote so UI always matches the gateway
            if (typeof j.amount === 'number' && j.currency) {
                setQuote({ amount: j.amount, currency: j.currency });
            }
            // small delay so users see the spinner lock, then go
